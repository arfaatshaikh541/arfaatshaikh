...
## Current milestone
**Milestone 9: Billing provider integration** — implementation complete, backend and frontend both. A real Stripe integration (not a fabricated payment flow): Checkout Sessions to subscribe/upgrade, Customer Portal sessions for self-service management, and a webhook handler that is the sole source of truth for subscription/invoice state — the `BillingCustomer`/`BillingSubscription`/`BillingEvent`/`InvoiceRecord` entities ADR-0006 explicitly deferred out of Milestone 1. No real Stripe account exists in this environment, so no live Checkout Session was ever opened against Stripe's real servers — but webhook signature verification is fully real (Stripe's HMAC-SHA256 scheme is public and offline-constructible), proven end-to-end with genuinely signed payloads through the real verification code path, not a bypassed one, and every outbound Stripe SDK call's exact request shape is verified against a monkeypatched call boundary, the same mocked-adapter discipline ADR-0010 (Google Places) established. Live browser verification confirmed the full UI: plan cards correctly distinguish "Upgrade" (a plan with a real Stripe price configured) from "Not yet available" (one without, never fabricated), and the honest "Billing is not configured for this deployment" error surfaces correctly with no live key set. See ADR-0017. Pending your review before proceeding to Milestone 10.
...
| Backend tests pass | ✅ 138/138 (api) + 72/72 (worker) + 22/22 (connector-sdk) = 232/232 |
...
## Pending approvals
None outstanding. Milestone 9's scope (billing provider integration, per ADR-0006's explicit deferral) was implemented under the standing "Proceed"-equivalent instruction ("Milestone 9"). ADR-0017 documents the full design — the real-Stripe-not-fabricated decision, the webhook-as-sole-source-of-truth principle, `SubscriptionPlan.stripe_price_id` as real never-fabricated per-deployment config, and the RLS/idempotency patterns reused from ADR-0007 for the new webhook-caller shape — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 9. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 10 started next, per the original architecture's milestone order.
