...
## Current milestone
**Milestone 11: RLS ordering guard (runtime enforcement)** — implementation complete, backend-only (this milestone has no frontend surface). No spec text named Milestone 11 directly; ADR-0007 and ADR-0018 (Milestone 10) both left the RLS *ordering* rule — every query against an RLS-protected table must run on a session that already had `set_tenant_context`/`set_platform_bypass` called on it — as an explicitly-named, still-open manual-discipline risk, since Milestone 10 only automated RLS *coverage* (every table has real RLS), not ordering (a call-site property no schema-introspection test can verify). This was chosen as Milestone 11's scope over the only other outstanding candidate — live verification against real Google Places/Stripe/webhook-receiver credentials — which isn't actionable in this environment. Delivered: an always-on SQLAlchemy runtime guard (`do_orm_execute`/`after_transaction_end` event listeners scoped to a dedicated `_AppSession` class, the class `AsyncSessionLocal` — the only production session factory in this codebase — is built on) that raises immediately, in every environment, the instant a query against an RLS-protected table runs on a session lacking a context marker, rather than letting it silently return zero rows the way Postgres's own RLS policy does. Building and validating it against the full existing test suite surfaced and fixed three false-positive classes (migrator-role test sessions, the `roles`/`role_permissions` tables' deliberately more-permissive policy, and an `audit_logs`/`platform_audit_logs` substring-collision bug) — none of which were real security bugs in the underlying application; the worker package required zero code changes and its full 72-test suite passed unmodified on the first correctly-scoped run. A dedicated 9-test regression file (`test_rls_ordering_guard.py`) proves each of the guard's specific behavioral properties directly. See ADR-0019. Pending your review before proceeding to Milestone 12.
...
| Backend tests pass | ✅ 150/150 (api) + 72/72 (worker) + 22/22 (connector-sdk) = 244/244 |
...
## Pending approvals
None outstanding. Milestone 11's scope (the RLS ordering guard, closing the specific gap ADR-0007 and ADR-0018/Milestone 10 both explicitly left open) was self-selected — no spec text in this session names Milestone 11 directly, matching Milestone 10's own situation — and implemented under the standing "Proceed"-equivalent instruction ("Milestone 11"), with the choice and its reasoning stated in prose before implementation, per the standing "explain major architecture decisions before implementation" rule. ADR-0019 documents the full design — the always-on-runtime-guard-over-static-lint decision, the event-listener mechanism, the `_AppSession`/`sync_session_class` scoping fix, and the two false-positive classes found and fixed while validating it — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 11. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 12 started next. No spec text in this session names Milestone 12's scope, the same situation Milestones 10 and 11 were both in — if you have a specific next milestone in mind, please share it; otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value gap to close (the strongest remaining candidate: a real live-verification pass against actual Google Places/Stripe/webhook-receiver credentials from an unrestricted network — every other named gap in this session's environment (Docker daemon access, general internet egress) has already been hit repeatedly and is unlikely to be resolvable from within this sandbox).
