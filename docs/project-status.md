...
## Current milestone
**Milestone 18: fix the seeded demo tenant's unusable email domain** — implementation complete, backend-only, no schema change. No spec text named Milestone 18 directly; self-selected as the last remaining known limitation that is both a genuine, actionable bug (not a by-design tradeoff) and credential-free — every other open item is blocked on external credentials or infrastructure (a Google Cloud API key, a Stripe test-mode key, general internet egress, a real MinIO/Docker deployment) this sandbox doesn't have. Every demo-tenant email address seeded since Milestone 1 (`app/seed/seed_data.py`'s `DEMO_TENANT_USERS`/`PLATFORM_DEMO_USER`) used a `.local` TLD, which Pydantic's `EmailStr` rejects as a reserved special-use domain at the request-validation layer — meaning the entire demo tenant had apparently never been able to log in through the real `/auth/login` endpoint, undetected for seventeen milestones because every prior live-verification pass worked around it by registering a fresh `@example.com`-style user instead. Fixed by changing the placeholder domain to `*.gridkeep-demo.example.com`/`gridkeep-platform-demo.example.com` — `example.com` is RFC 2606's reserved documentation domain, a real non-special-use TLD `EmailStr` accepts, while remaining obviously fictional. Live-verified with a real `POST /auth/login` against both the tenant-owner and platform-admin demo accounts after re-seeding the dev database — both now return `200` with a real session cookie. See ADR-0026. Pending your review before proceeding to Milestone 19.
...
| Backend tests pass | ✅ 154/154 (api) + 85/85 (worker) + 26/26 (connector-sdk) = 265/265 |
...
## Pending approvals
None outstanding. Milestone 18's scope (fixing the seeded demo tenant's unusable email domain) was self-selected as the last remaining known limitation that is both a genuine, actionable bug and credential-free — every other open item (Google Places live verification, Stripe live verification, a live crawl, live webhook delivery, a real MinIO/Docker deployment) is blocked on external credentials or infrastructure this sandbox doesn't have. ADR-0026 documents the full design — the placeholder-domain-not-validation-rule decision, why `example.com` was chosen, the additive (not destructive) re-seeding behavior, and the live-login verification — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 18. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 19 started next. No spec text in this session names Milestone 19's scope, the same situation every milestone since 10 has been in. Most remaining open items are either deliberate by-design tradeoffs or blocked on external credentials/infrastructure this sandbox does not have (a real Google Cloud API key, a real Stripe test-mode key, general internet egress, a real MinIO/Docker deployment). One credential-free, actionable gap remains: limitation #18, a "score all businesses from this campaign" bulk action (scoring is currently manual, one business at a time, via `POST /businesses/{id}/score`) — the next candidate if no new scope or credentials are provided.
