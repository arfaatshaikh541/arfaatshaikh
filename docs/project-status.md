...
## Current milestone
**Milestone 14: export storage cleanup** — implementation complete, backend-only (this milestone has no frontend surface). No spec text named Milestone 14 directly; chosen as the next highest-value credential-free gap under the user's "Milestone 14" authorization. Closes a known limitation carried since Milestone 7: `Export.expires_at` was documented as "informational only, no cleanup job consumes it yet" — exported files persisted in object storage indefinitely. Delivered: a real, configurable `EXPORT_RETENTION_DAYS` setting (default 30, disclosed as a chosen operational default, not a fabricated requirement) now sets a real `expires_at` on every export's completion; a new hourly Celery beat sweep (`worker.export_cleanup_tasks`, mirroring `expire_stale_reservations`'s exact shape) deletes each expired export's underlying object once its retention period passes and marks a new `Export.storage_deleted_at` column — the sweep's own idempotency key, since S3's `DeleteObject` is itself idempotent against an already-absent key, verified directly with a test that simulates a crash between the delete and the mark. The download endpoint now also rejects a request for an export whose file has already been cleaned up, rather than presigning a URL to a deleted object. See ADR-0022. Pending your review before proceeding to Milestone 15.
...
| Backend tests pass | ✅ 154/154 (api) + 81/81 (worker) + 22/22 (connector-sdk) = 257/257 |
...
## Pending approvals
None outstanding. Milestone 14's scope (export storage cleanup) was self-selected under the user's bare "Milestone 14" authorization as the next highest-value credential-free gap — a real, long-standing, explicitly-named limitation (#23, carried since Milestone 7) that needed no live external service to implement or test for real, since object storage has been fully testable via `moto`'s real local S3-API server since Milestone 7 (ADR-0015). ADR-0022 documents the full design — the real-not-fabricated retention-period setting, the beat-sweep design mirroring `expire_stale_reservations` exactly, the `storage_deleted_at`-plus-idempotent-`DeleteObject` idempotency design (verified directly against a simulated crash window), and the download-path's new cleanup check — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 14. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 15 started next. No spec text in this session names Milestone 15's scope, the same situation every milestone since 10 has been in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value credential-free gap to close.
