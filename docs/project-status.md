...
## Current milestone
**Milestone 20: OpenStreetMap/Overpass connector** — implementation complete, full-stack, no schema change. Unlike every milestone since 10, this one was not self-selected — the user directly asked "How about we use OpenStreetMap + Overpass API" after being told Google Places (the only other real connector) needs a paid, credentialed Google Cloud API key. Added `OverpassConnector`, a third `BaseConnector` implementation registered under `"osm"`, needing no credential of any kind — real business search against OpenStreetMap's free Overpass API. New `BaseConnector.supports_rating_filter` flag (`False` for this connector, unchanged default `True` for the others) makes campaign creation reject `min_rating`/`min_reviews` outright for this source rather than silently ignoring them, since OSM has no rating/review schema at all. An industry→OSM-tag mapping table covers ~30 common categories; an unmapped industry is rejected with a clear error rather than silently returning nothing. The campaign-creation form gained a "Data source" selector (previously hardcoded to `mock`), with a caption that switches per source. Confirmed via direct `curl` that this sandbox's egress proxy blocks every Overpass/OpenStreetMap host tried, the same class of restriction the crawler and Stripe already carry — so, like Google Places, this is verified via 23 mocked-HTTP-transport tests, not a live call. Live-verified in a real browser regardless: created a real `osm`-sourced campaign, confirmed the rating-filter rejection (client-side and server-side), and **launched it for real** — the genuinely-blocked network call correctly retried through the existing retry/backoff machinery and reached a clean `"failed"` state with the real error recorded and the full credit reservation released, proving the failure path end-to-end even though the success path against real Overpass data remains unverified. See ADR-0028. Pending your review before proceeding to Milestone 21.
...
### RLS ordering guard (`apps/api/app/core/db.py`) — closes ADR-0007/ADR-0018's named ordering gap
An always-on SQLAlchemy runtime guard, not a static lint rule: a source-level analyzer would need real control-flow analysis to avoid both false positives and false negatives, and even a perfect version would only catch mistakes in code someone remembered to lint. Instead, two ORM event listeners — `do_orm_execute` (fires for every `session.execute()` call, including raw `text()` statements, not just ORM-constructed `select()`/`insert()`) and `after_transaction_end` — are registered on a dedicated `_AppSession` class. `AsyncSessionLocal` (confirmed, via `grep -rln "async_sessionmaker("`, to be the only session factory anywhere in this codebase's production code) is built with `sync_session_class=_AppSession`, so the guard protects every real request/task session. On each query, the listener checks a `session.info` marker set by `set_tenant_context`/`set_platform_bypass`; if absent and the statement touches a table with a `tenant_id` column (computed from `Base.metadata`, not a hardcoded list — the same schema-introspection principle `test_rls_coverage.py` established in Milestone 10), it raises `RuntimeError` immediately, before the query ever reaches Postgres, instead of letting RLS silently return zero rows. The marker is cleared on `after_transaction_end` only when the *outer* transaction ends (`transaction.parent is None`) — never on a nested `SAVEPOINT` ending, so CSV import's per-row `session.begin_nested()` pattern (Milestone 9) is unaffected.

### Two false-positive classes found and fixed while validating against the full test suite
Attaching the guard to the global `Session` class first produced 55 failures: this suite's own service-level tests build ad-hoc sessions connecting as the `gridkeep_migrator` role (`BYPASSRLS`), so lacking the Python-level marker there is genuinely harmless — fixed by the `_AppSession` scoping above. After that fix, 3 failures remained in `test_platform_admin.py`, tracing to `roles`/`role_permissions`: direct `pg_policies` inspection confirmed these two tables carry a deliberately more permissive policy (`tenant_id = current_tenant_id OR tenant_id IS NULL OR platform_bypass`) than the other ~44 RLS-protected tables, specifically so platform-role catalog rows are readable with zero context — legitimate, pre-existing, correct design. Both were added to a documented `_GUARD_EXCLUDED_TABLES` set alongside `platform_audit_logs` (already excluded per ADR-0018's own reasoning). One further failure — `list_platform_audit_logs` wrongly blocked because the excluded table name `platform_audit_logs` contains the guarded table name `audit_logs` as a literal substring — was fixed by replacing a naive substring-match loop with a single precompiled word-boundary regex (`\b(?:table1|table2|...)\b`), verified against a scan of all 47 RLS-tracked table names to be the only such collision.

### Tests
`apps/api/tests/test_rls_ordering_guard.py` (new, 9 tests) — blocks a context-less query; permits one after `set_tenant_context`/`set_platform_bypass`; re-blocks after the setting transaction commits; does not re-block across a nested SAVEPOINT rollback (the CSV-import shape); does not false-positive on `platform_audit_logs` or `roles`/`role_permissions` (regression tests for the two bugs found above); does not affect a session from an unrelated `async_sessionmaker` (scoping proof); blocks a raw `text()` query exactly as it blocks an ORM-constructed one. Full monorepo verification: 150/150 (api) + 72/72 (worker) + 22/22 (connector-sdk) = 244/244 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change this milestone); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all clean (unchanged this milestone — no frontend files touched).

## Milestone 12 completed work

### Environment finding: Google Places is now network-reachable (not yet actionable)
Before choosing this milestone's scope, connectivity was re-checked given the standing recommendation to pursue live verification next. `curl` to `https://places.googleapis.com/v1/places:searchText` with an invalid key returned a genuine `400 INVALID_ARGUMENT`/`API_KEY_INVALID` JSON body from Google's own servers — confirmed via the proxy's own status endpoint that this host is not in the recent-relay-failures list, unlike `api.stripe.com` and `example.com`, which are still rejected with policy `403`s. This is a real change from every prior milestone's disclosed gap (ADR-0010). It does not close the gap by itself: no Google Cloud API key exists anywhere in this repo's `.env` files, and one was not fabricated. This is recorded here so a future session with a real key available knows the network path is already open.

### Shared Celery retry-exhaustion helper (`apps/worker/worker/retry.py`) — closes ADR-0016's named follow-up risk
`retry_or_finalize(task, *, exc, finalize, countdown, task_name, task_id)` is now the one sanctioned way any Celery task in this codebase retries-then-finalizes: if `task.request.retries >= task.max_retries`, it logs a consistently-named `task_retries_exhausted` event and runs `finalize()` (a domain-specific coroutine that marks the record `"failed"` with a real error) without raising; otherwise it raises `task.retry(exc=exc, countdown=countdown)` exactly as each task did by hand before. `default_backoff(retries)` extracts the shared `min(60, 5 * (2**retries))` formula all five tasks already used. All five task files (`campaign_tasks.py` — both its `_SlotUnavailable` and connector-error branches — `enrichment_tasks.py`, `export_tasks.py`, `csv_import_tasks.py`, `integration_tasks.py`) were migrated to call it, replacing five hand-written copies of the identical exhaustion check with one shared function that a future task literally cannot get wrong by omission — the old broken shape (`try/except MaxRetriesExceededError`) is no longer something any task needs to remember not to write.

### Two issues found and fixed while migrating
Ruff's `F821` flagged every lambda built inside an `except SomeError as exc:` block that referenced `exc` directly — a genuine Python semantic, not a lint false positive: `except ... as exc` implicitly deletes `exc` when the block exits (to avoid a reference-cycle-holding traceback), so a closure capturing it is unsafe in general even though it happened to work here (the lambda is invoked synchronously, before the block exits). Fixed by assigning `error = exc` to a plain local and closing over that instead, at all four affected call sites; documented in `worker/retry.py`'s own docstring for future callers. Separately, `test_integration_tasks.py`'s retry-exhaustion regression test monkeypatched `run_db_task` at its old call site (`integration_tasks.py` itself); after migration the real finalize-path call moved into `worker.retry`, which holds its own separate import of `run_db_task` — the test needed an additional monkeypatch of `worker.retry.run_db_task` to keep working. No other existing test in the suite monkeypatches `run_db_task` (confirmed via a grep across every test file), so this was the only test requiring a change.

### Tests
`apps/worker/tests/test_retry.py` (new, 5 tests) — exercises `retry_or_finalize` directly against a minimal fake Celery task double: retries with `exc`, retries without `exc` (the `_SlotUnavailable` shape), finalizes-and-does-not-raise on exact exhaustion, finalizes defensively when retries already exceed the budget, and `default_backoff`'s exponential-capped-at-60 formula. `apps/worker/tests/test_integration_tasks.py` — one existing test updated (its `run_db_task` monkeypatch target, not its assertions). Full monorepo verification: 150/150 (api, unchanged) + 77/77 (worker) + 22/22 (connector-sdk) = 249/249 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change this milestone); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all re-run and clean, unchanged (19 routes generated — no frontend files touched this milestone).

## Milestone 13 completed work

### Recurring credit grant on billing renewal (`apps/api/app/modules/billing`) — closes the "no recurring credit grant" known limitation
`_grant_recurring_credits_for_invoice` runs from `_handle_invoice_event` whenever a webhook-delivered invoice is `"paid"` and carries a `subscription` id (one-off, non-subscription invoices are skipped). It resolves the plan from the invoice's own first line item's `price.id` — not the already-synced `TenantSubscription` row, since Stripe does not guarantee event delivery order between an invoice event and its sibling subscription event for the same cycle — and grants that plan's `monthly_credit_grant` via the existing `usage.services.grant_credits` (the same `"grant_recurring"` transaction type the one-time trial grant already uses). Every paid subscription invoice grants credits, not just later renewals: a tenant's Milestone-1 initial grant is for the *trial* plan specifically, made at signup before any real subscription exists, so the first paid invoice for whatever plan a tenant actually checks out into is its own distinct billing period, owed the same as every one after it.

### Idempotency keyed on the Stripe invoice id, not just the event id
The webhook handler's existing `BillingEvent`-id dedup happens before any side effects run, but its own dedup row is only written at the very end of `handle_webhook_event` — a crash between granting credits and that final write would leave nothing to stop a genuinely redelivered event (which Stripe's own docs warn can arrive under a different event id for the same object) from granting a second time. A new nullable `InvoiceRecord.credit_grant_applied_at` column, read through a new locked `get_invoice_by_stripe_id_for_update` (`SELECT ... FOR UPDATE`) immediately before being checked and set in the same transaction the grant itself commits in, closes both the crash-window gap and a genuine concurrent-delivery race. Proven directly, not just reasoned about: a new test sends two genuinely, correctly HMAC-signed webhook payloads for the *same* invoice id under two *different* event ids against the real test database, and asserts the tenant's wallet balance moved exactly once.

### Tests
`apps/api/tests/test_billing_api.py` (+3 tests) — grants the configured plan's credit amount on a paid subscription invoice; does not double-grant when the same invoice is redelivered under a different event id; does not crash or fabricate a grant for an invoice whose price isn't mapped to any configured plan. All three use the same real, genuinely-signed-webhook-payload discipline ADR-0017 established — no mocked signature verification. Full monorepo verification: 153/153 (api) + 77/77 (worker, unchanged) + 22/22 (connector-sdk, unchanged) = 252/252 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift; frontend `pnpm lint`, `pnpm typecheck`, and `next build` all re-run and clean, unchanged (19 routes — no frontend files touched this milestone).

## Milestone 14 completed work

### Export storage cleanup (`apps/api/app/core/storage.py`, `app/modules/exports`, `apps/worker/worker/export_cleanup_tasks.py`) — closes the "no storage-cleanup job" known limitation
A new `EXPORT_RETENTION_DAYS` setting (default 30) is a real, disclosed operational default — no spec ever specified a retention period, so this is a documented choice, not a fabricated requirement. `worker.export_tasks.run_export` now sets `Export.expires_at = completed_at + retention_period` on every successful completion, instead of always `None`. A new `worker.export_cleanup_tasks.cleanup_expired_exports_task`, registered on the existing `queue.maintenance` queue and scheduled hourly via Celery beat, mirrors `expire_stale_reservations`'s exact shape: a thin task wrapping a cross-tenant service function (`exports.services.cleanup_expired_exports`) that runs under `set_platform_bypass`, finds every completed export whose `expires_at` has passed and whose storage hasn't been cleaned up yet, deletes the underlying object via a new `storage.delete_object`, and marks a new `Export.storage_deleted_at` column.

### Idempotency verified directly, not just by construction
`storage_deleted_at` is the sweep's own idempotency key (only unset rows are ever selected), and `delete_object` relies on S3's own protocol guarantee that `DeleteObject` succeeds silently against an already-absent key — meaning a retry after a crash between the delete and the mark is always safe. This was proven, not just reasoned about: a dedicated test pre-simulates exactly that crash window (the object already gone, the row not yet marked) and confirms the sweep completes cleanly and marks the row correctly on retry.

### The download path now checks for cleanup
`exports.services.get_download_url` previously only checked `status == "completed"`. It now also rejects a request once `storage_deleted_at` is set, with a clear error telling the requester to create a new export — rather than silently presigning a URL to an object that no longer exists.

### Tests
`apps/worker/tests/test_export_cleanup_tasks.py` (new, 4 tests) — deletes the object and marks the row for a genuinely expired export; skips an export whose retention hasn't expired yet; skips an export already cleaned up (excluded by the query, not re-processed); and the idempotency-under-a-crash-window test described above. `apps/api/tests/test_exports.py` (+1 test) — the download-rejection-after-cleanup case. Full monorepo verification: 154/154 (api) + 81/81 (worker) + 22/22 (connector-sdk) = 257/257 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift; frontend `pnpm lint`, `pnpm typecheck`, and `next build` all re-run and clean, unchanged (19 routes — no frontend files touched this milestone).

## Milestone 15 completed work

### `FaultInjectingConnector` (`packages/connector-sdk/connector_sdk/fault_injecting.py`) — closes the "no connector-error-path test coverage" gap
A small connector wrapping `MockConnector`: a scripted `outcomes` sequence lets a test say exactly which call to `search()` should raise which exception (or delegate to a real `MockConnector` call), then falls back to always succeeding once the script is exhausted. Deliberately not exported from the package's public surface and not registered in `connector_sdk.registry`'s production connector map — it must never be selectable as a real campaign's `source_key`. Tests wire it in by monkeypatching the calling task module's own `get_connector` reference.

### Two real bugs found and fixed while building the test this gap asked for
Driving `worker.campaign_tasks.run_campaign_task` through a real Celery retry (not just calling `_run_campaign_task_async` directly, the pattern every existing worker test used before this) surfaced a genuine production bug: `lock_task_for_processing` commits the task to `"running"` *before* the connector call; a transient/rate-limit failure there triggers a real retry, but nothing ever reset the status back to `"pending"` — so the real retried delivery hit the task's own duplicate-delivery guard (`status != "pending"`) and silently no-op'd, burning the entire retry budget doing nothing before eventually, correctly, finalizing as failed. Checking the other four retrying tasks for the identical shape (a claimable-state field committed before the task's own retryable failure point) found the same bug in `worker.enrichment_tasks` (confirmed with its own dedicated test showing the identical symptom before being fixed) — and confirmed `export_tasks`, `csv_import_tasks`, and `integration_tasks` do not share it: one avoids it by never committing an intermediate status change before its own retry point, and two were deliberately designed around it already, with in-code comments explaining why. Both real bugs are now fixed identically: reset the claimable-state field back to `pending` immediately before scheduling a genuine retry (never before finalizing as failed, to avoid opening a race window with another worker process).

### A pre-existing test-infrastructure gap found and fixed along the way
Running a real Celery-wrapped task (via `worker.async_utils.run_db_task`, a fresh event loop per call) in the same test process as ordinary tests that call `_run_..._async` directly (sharing pytest-asyncio's own session-scoped loop) turned out to have a latent cross-loop bug: the production DB engine/redis client accumulate connections bound to that shared session loop from the ordinary-style tests, and the first real `run_db_task` call in the whole test session then tries to close them from an unrelated fresh loop. A new `real_celery_task_isolation` fixture (`apps/worker/tests/conftest.py`) disposes both while still running on the loop that actually owns them, closing the gap for any future test using this pattern.

### Tests
`apps/worker/tests/test_campaign_task_retry_loop.py` (new, 3 tests) — transient failure retries then succeeds (the test that caught Bug 1); retries-exhausted finalizes cleanly with the reservation fully released (never covered through the real Celery wrapper before); an auth error fails immediately with no retry attempted. `apps/worker/tests/test_enrichment_task_retry_loop.py` (new, 1 test) — the equivalent transient-retry-then-succeeds proof for Bug 2. `packages/connector-sdk/tests/test_fault_injecting.py` (new, 4 tests) — the connector's own scripting/delegation/exhaustion behavior. Full monorepo verification: 154/154 (api, unchanged) + 85/85 (worker) + 26/26 (connector-sdk) = 265/265 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change this milestone); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all re-run and clean, unchanged (19 routes — no frontend files touched this milestone).

## Milestone 16 completed work

### Website enrichment UI (`apps/web/app/(tenant)/leads/[id]/page.tsx`) — closes the "no frontend UI for enrichment" known limitation
A new "Website enrichment" card sits right after the business-info card on the lead detail page: an "Enrich website"/"Re-enrich website" trigger button (disabled while a run is already in progress, or replaced with an explanatory message when the business has no website on file), a live status badge polled every 2s while non-terminal (the same pattern the campaign progress panel already established), and — once completed — the pages-crawled count, last-run timestamp, and every recorded evidence row (detector type, source URL, confidence, supporting snippet). A failed run shows its real error message. No backend or worker code changed — every endpoint this UI calls (`POST /businesses/{id}/enrich`, `GET .../enrichment`, `GET .../evidence`) has existed unchanged since Milestone 4.

### Live-verified against the real running stack
Per the standing rule for UI changes, this was driven through a real `uvicorn`/Celery-worker/`next dev` stack in headless Chromium (Playwright), not just typechecked: registered a fresh user, verified email via a captured SMTP message, onboarded a workspace, launched a real mock campaign to completion via the live worker, scored a discovered business with a website into a lead, then clicked "Enrich website" on the actual rendered page and watched it poll from no-status through `completed`, rendering a real evidence row (`website_unavailable`, since `MockConnector`'s fabricated `.example.com`-style domains don't resolve — a correct, honest outcome, not a bug). Screenshots of both states were captured and visually inspected.

### An unrelated, real bug found and disclosed (not fixed this milestone)
Setting up for that verification, logging in as the seeded demo tenant's `owner@northstar-demo.gridkeep.local` through the real `/auth/login` endpoint failed with a generic validation error — traced to Pydantic's `EmailStr` rejecting `.local` as a reserved, special-use TLD. Every demo-tenant email address seeded since Milestone 1 uses this TLD, meaning the entire demo tenant has apparently never been able to log in through the real API — every prior milestone's own live-verification pass registered a fresh `@example.com`-style user instead, which is presumably why this went unnoticed for fifteen milestones. Recorded as a new known limitation; not fixed here since it's unrelated to this milestone's actual scope.

### Tests
None — this is a pure UI addition over already-tested backend endpoints; `pnpm lint`/`pnpm typecheck`/`pnpm build` plus the live browser pass above are the verification. Full monorepo verification: 154/154 (api, unchanged) + 85/85 (worker, unchanged) + 26/26 (connector-sdk, unchanged) = 265/265 passing; ruff and mypy clean; `alembic check` reports no drift (no schema change); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all clean (19 routes — no new route, an existing page gained a new card).

## Milestone 17 completed work

### Duplicate-candidate review UI (`apps/web/app/(tenant)/leads/[id]/page.tsx`) — closes the "duplicate review buttons not wired up" known limitation
The existing Duplicate Candidates card on the lead detail page now shows "Confirm merge"/"Reject" buttons for each `pending` candidate (using the page's existing `runAction`/`pending`/`actionError` pattern every other action on this page already uses), and a resolved status/confidence/reviewed-at line for already-reviewed ones. A new Merge History card — nothing previously rendered merge history in the frontend at all — lists confirmed merges for the lead's business with an "Undo" button while `undone_at` is null. Both cards resolve the "other business"'s real name (neither `DuplicateCandidateResponse` nor `MergeHistoryResponse` includes one, only IDs) via a batched `useQueries` lookup against the existing `GET /businesses/{id}` endpoint. No backend or worker code changed — every endpoint this UI calls (`POST /duplicate-candidates/{id}/confirm|reject`, `POST /merges/{id}/undo`, `GET /businesses/{id}/merge-history`) has existed unchanged since Milestone 5/6.

### Live-verified against the real running stack, including a genuine fuzzy-match duplicate
Per the standing rule for UI changes, this was driven through a real `uvicorn`/Celery-worker/`next dev` stack in headless Chromium (Playwright). A real `pending` duplicate candidate (not fabricated data) was produced by seeding one business via a mock campaign, then CSV-importing a second business with a ~90%-similar name in the same city and a different phone — landing in the `fuzzy_name` matching tier (max confidence 0.75, structurally below the 0.85 auto-merge threshold), which reliably stays `pending`. CSV import's synchronous upload step needed a real S3-compatible endpoint; since this sandbox has no Docker daemon (confirmed in prior milestones, ADR-0015), a standalone `moto.server.ThreadedMotoServer` — the same approach `apps/api/tests/conftest.py`'s own `moto_s3` fixture already uses for the identical reason — was run as a persistent process for the verification pass. The script then confirmed the candidate on the real page (watching status update and the Merge History card appear with correct resolved names) and undid it (watching the entry flip to "undone" and the Undo button disappear). Screenshots of all three states were captured and visually inspected.

### Tests
None — this is a pure UI addition over already-tested backend endpoints; `pnpm lint`/`pnpm typecheck`/`pnpm build` plus the live browser pass above are the verification. Full monorepo verification: 154/154 (api, unchanged) + 85/85 (worker, unchanged) + 26/26 (connector-sdk, unchanged) = 265/265 passing; ruff and mypy clean; `alembic check` reports no drift (no schema change); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all clean (19 routes — no new route, an existing page gained one updated card and one new card).

## Milestone 18 completed work

### Seeded demo tenant email domain fix (`apps/api/app/seed/seed_data.py`) — closes the "demo tenant can't log in" known limitation
Every demo-tenant email address (`DEMO_TENANT_USERS`, `PLATFORM_DEMO_USER`) changed domain from `*.gridkeep.local`/`gridkeep-platform-demo.local` to `*.gridkeep-demo.example.com`/`gridkeep-platform-demo.example.com`. `.local` is a reserved special-use TLD that Pydantic's `EmailStr` rejects outright at request-validation time, before any service code runs — so `POST /auth/login` for any seeded demo account always 422'd, apparently since Milestone 1. `example.com` is RFC 2606's reserved documentation domain: a real, non-special-use TLD `EmailStr` accepts, while the address remains obviously fictional. No schema change, no migration — this is a seed-data-only fix. `core/config.py`'s unrelated `smtp_from_email` default (also `.local`, but `str`-typed and only ever used as an outbound SMTP "From" header, never request-validated) was deliberately left untouched.

### Live-verified with a real login
Restarted the real `uvicorn` server, re-ran the seed script against the dev database (its existing idempotency-by-email-natural-key behavior means the old `.local` rows are left in place, undisturbed, alongside the new ones — not renamed or deleted), and sent real `POST /auth/login` requests for both the tenant-owner and platform-admin demo accounts under their new addresses — both returned `200` with a real session cookie, proving the fix works through the actual validation layer that originally rejected them. The old `.local` address was re-tested and still correctly 422s, confirming it's inert leftover data, not a security concern.

### Tests
No new automated tests — no test in the checked-in suite ever asserted anything about these specific seeded email addresses (confirmed by search before making the change), so there was nothing to update; the live login check above is the verification, matching the shape of this bug (a request-validation-layer rejection, best proven by a real request). Full monorepo verification: 154/154 (api, re-run against the changed seed data) + 85/85 (worker, unchanged) + 26/26 (connector-sdk, unchanged) = 265/265 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all re-run and clean, unchanged (19 routes — no frontend files touched this milestone).

## Milestone 19 completed work

### Bulk lead scoring (`app/modules/leads/scoring.py`, `app/modules/campaigns/routes.py`, `apps/web/app/(tenant)/campaigns/[id]/page.tsx`) — closes the "scoring has no bulk trigger" known limitation
`scoring.score_businesses_for_campaign(session, campaign_id)` scores every canonical (non-merged-away) business a campaign discovered, one at a time through the existing `score_lead` (pure computation, no network), returning the full per-business result list without committing — the caller commits once. `POST /campaigns/{campaign_id}/score-businesses` (permission `leads.score`, the same one the single-business endpoint already uses) exposes it. `ScoreLeadResponse` gained a `from_score_result` classmethod, extracted from the single-business route's own inlined response-building code (a refactor, not a behavior change — that route's existing tests still pass unmodified) and reused by both routes. The frontend gained one "Score all businesses" button on `/campaigns/[id]`, shown once `progress.businesses_found > 0` (not gated on a terminal campaign status, since scoring needs no crawl/network), calling the new endpoint and showing a "Scored N businesses into leads." banner.

### Deliberately a bulk action, not an automatic trigger
The known limitation named two possible follow-ups: a bulk action, or an automatic post-discovery scoring trigger. This milestone built only the former — an automatic trigger would change every tenant's campaign behavior unconditionally (every discovered business becomes a scored Lead the instant it's found) with no way to opt out, a larger change than the disclosed gap asked for. The bulk action is explicit and user-initiated, consistent with this project's standing "don't invent requirements beyond what's captured" rule.

### Live-verified against the real running stack
Per the standing rule for UI changes, this was driven through a real `uvicorn`/Celery-worker/`next dev` stack in headless Chromium (Playwright): registered a fresh user, launched a real mock campaign to completion via the live worker (8 businesses discovered), confirmed the "Score all businesses" button is genuinely absent before discovery and appears once the campaign has found businesses, clicked it, watched a real "Scored 8 businesses into leads." success banner, then navigated to `/leads` and confirmed all 8 businesses appear there as distinctly-scored leads (different real scores, not a placeholder). Screenshots of all three states were captured and visually inspected.

### Tests
Two new service-level tests (`test_lead_scoring.py`): the bulk function scores every canonical business discovered by a campaign, and correctly excludes a business that lost a dedup merge (only the winner is scored, matching `list_businesses_for_campaign`'s existing canonical-only contract). One new HTTP-level test (`test_campaign_engine.py`): the route returns `200`/`[]` for an owner with no businesses yet discovered (no worker runs in the API-only test process) and `403 permission_denied` for a Read-Only Viewer. Full monorepo verification: 157/157 (api, +3 from this milestone) + 85/85 (worker, unchanged) + 26/26 (connector-sdk, unchanged) = 268/268 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all clean (19 routes — no new route, an existing page gained one button).

## Milestone 20 completed work

### OpenStreetMap/Overpass connector (`packages/connector-sdk/connector_sdk/overpass.py`) — a real, credential-free third data source
`OverpassConnector` implements `BaseConnector` exactly like `MockConnector`/`GooglePlacesConnector`, registered under `"osm"` — no API key, no signup, no card on file, unlike Google Places. Builds real Overpass QL queries scoped to a named country→city administrative area, mapping the campaign's industry/category to OSM's own tagging scheme (`amenity=restaurant`, `shop=car_repair`, etc.) via a ~30-entry table; an unmapped industry is rejected with a clear error rather than silently returning nothing. Every field traces to a real OSM tag or a documented, defensible fallback — rating/review_count are always `None` (OSM has no such schema, never fabricated), business_status stays `None` (unknown) unless a `disused:`/`abandoned:` tag says otherwise, and a missing name falls back to the record's own stable OSM id, never an invented name. No server-side pagination exists in Overpass, so this connector re-queries deterministically and slices per page, the same "recompute, slice by page" contract `MockConnector` already uses.

### `BaseConnector.supports_rating_filter` — reject, don't silently ignore
OSM has no rating/review data at all. Rather than accepting a campaign's `min_rating`/`min_reviews` filter and never enforcing it (which would look like it worked while doing nothing), a new `supports_rating_filter` flag on `BaseConnector` (default `True`, unchanged for the other two connectors; `False` for `OverpassConnector`) makes `campaigns.services.create_campaign` reject that combination outright with a clear validation error — enforced server-side (authoritative) and mirrored client-side in the campaign form's zod schema for immediate feedback.

### Frontend: a real "Data source" selector (previously hardcoded to mock)
The campaign-creation form gained a select ("Mock (sample data)" / "OpenStreetMap (real, free)", mock still the default) with a caption that switches to describe whichever source is selected, including the no-rating-data disclosure for OpenStreetMap. `google_places` remains unexposed in this form (known limitation #9, unchanged) — it still needs a credential this environment doesn't have; OSM has no such barrier, which is exactly why it's exposed.

### Live-verified, including the honest failure path
Confirmed via direct `curl` before writing any connector code that this sandbox's egress proxy rejects every Overpass/OpenStreetMap host tried (`overpass-api.de`, `overpass.kumi.systems`, `nominatim.openstreetmap.org`) with a policy-level 403 — the same class of restriction the crawler and Stripe already carry. Verification is therefore 23 mocked-HTTP-transport tests (mirroring Google Places' own 22), not a live call. Live-verified in a real browser regardless: confirmed the Data source selector and its caption, confirmed the rating-filter rejection both client-side and server-side, created a real `osm`-sourced campaign, and **launched it for real** — the genuinely-blocked network call correctly retried through the existing retry/backoff machinery (`worker/retry.py`'s `retry_or_finalize`, unchanged) and reached a clean `"failed"` terminal state with the real error (`ConnectorTransientError: Overpass request failed: 403 Forbidden`) recorded in both the Errors list and status history, and the full credit reservation released (wallet balance confirmed unchanged via `GET /usage/wallet`). This proves the failure path end-to-end for a brand-new connector; the real-query success path against actual Overpass data remains unverified, the same disclosed shape of gap as Google Places.

### Tests
`packages/connector-sdk/tests/test_overpass.py` (23 new tests) — successful parsing with no fabricated fields, way/relation coordinate handling via `out center`, disused/abandoned lifecycle tagging, unmapped-industry rejection with zero network calls, pagination slicing across multiple calls (including result_limit capping ahead of real available matches), client-side phone/website filtering, confirmation that min_rating/min_reviews are never applied even if passed directly to the connector, every mapped error class (429/5xx/400/non-JSON body/timeout), `estimate_cost`/`health_check`, and direct query-construction assertions (country→city area chaining, category-over-industry precedence). `apps/api/tests/test_campaign_engine.py` (2 new tests) — the rating-filter rejection at HTTP level, and successful `osm`-sourced campaign creation without those filters. Full monorepo verification: 159/159 (api, +2) + 85/85 (worker, unchanged) + 49/49 (connector-sdk, +23) = 293/293 passing; ruff and mypy clean across all three Python packages; `alembic check` reports no drift (no schema change); frontend `pnpm lint`, `pnpm typecheck`, and `next build` all clean (19 routes — no new route, an existing page gained a source selector).
...
| Backend tests pass | ✅ 159/159 (api) + 85/85 (worker) + 49/49 (connector-sdk) = 293/293 |
| Celery retry-exhaustion handling centralized and guarded against reintroduction | ✅ `worker/retry.py`'s `retry_or_finalize` (Milestone 12, ADR-0020) is the one sanctioned way any of this worker's five retrying tasks checks retry-budget exhaustion; 5 dedicated tests (`test_retry.py`) plus every task's own existing retry-exhaustion coverage, unmodified except one test's monkeypatch target |
| Billing: recurring credit grant on subscription renewal | ✅ `_grant_recurring_credits_for_invoice` (Milestone 13, ADR-0021) grants the invoiced plan's `monthly_credit_grant` on every paid subscription invoice, idempotent per Stripe invoice id (not just event id); idempotency proven against a genuine redelivered-under-a-different-event-id scenario, not just declared |
| Exports: storage cleanup of expired files | ✅ `worker.export_cleanup_tasks` (Milestone 14, ADR-0022) deletes a completed export's object once its retention period passes and marks `storage_deleted_at`; idempotency-under-a-crash-window proven directly by test, not just by construction |
| Connector error-path retry coverage through the worker's own retry loop | ✅ `FaultInjectingConnector` (Milestone 15, ADR-0023) drives `run_campaign_task`'s real Celery retry loop; found and fixed two real bugs (`campaign_tasks`, `enrichment_tasks`) where a transient failure left the task stuck, silently no-opping every subsequent retry instead of re-attempting the work |
...
| RLS ordering enforced automatically at runtime | ✅ Always-on `do_orm_execute`/`after_transaction_end` guard (`app/core/db.py`, Milestone 11, ADR-0019) raises immediately on any context-less query against an RLS-protected table, in every environment — not just a lint-time check; 9 dedicated regression tests (`test_rls_ordering_guard.py`); documented exception for `roles`/`role_permissions`' tenant-scoped half (see Known limitations) |
...
| OpenStreetMap/Overpass: real, credential-free connector | ✅ Milestone 20 (ADR-0028) — `OverpassConnector`, registered as `osm`, exposed in the campaign-creation form's new Data source selector; 23 mocked-adapter tests; live-verified failure path (real network attempt against this sandbox's blocked egress correctly retries then fails cleanly, full credit release confirmed) — real-query success path not live-verified, same shape of gap as Google Places |
...
| Website enrichment: frontend trigger and evidence viewing UI | ✅ Milestone 16 (ADR-0024) — a "Website enrichment" card on `/leads/[id]`: trigger button, live status polling, rendered evidence list; live-verified against the real running stack in a headless browser |
...
| Lead scoring: bulk scoring | ✅ Milestone 19 (ADR-0027) — `POST /campaigns/{id}/score-businesses` scores every canonical business a campaign discovered in one call; a "Score all businesses" button on the campaign detail page; live-verified against a real 8-business campaign |
...
| Lead workspace: duplicate review | ✅ Milestone 17 (ADR-0025) — Confirm/Reject buttons and a new Merge History card with Undo, wired to the Milestone 5/6 confirm/reject/undo endpoints; live-verified against a real fuzzy-match candidate |
...
See `docs/adr/0001` through `0028`. Summary: shared-schema+RLS multi-tenancy, server-side sessions, `uv`/`pnpm` tooling, campaign-level credit-reservation granularity, MFA scaffolded only, no billing provider selected yet as of Milestone 1 (0006, closed out by 0017), a documented RLS ordering rule + `platform_bypass` escape-hatch pattern (0007, its named "CI check" gap closed by 0018's coverage half and 0019's ordering half), campaign entity consolidation vs. the original architecture's larger entity list (0008), a documented per-task event-loop isolation rule for the worker's async DB/Redis clients (0009), the Google Places connector's design plus its explicit live-verification gap (0010), the Business entity consolidation and the Milestone 2 persistence gap it fixes (0011), the SSRF-safe crawler's design plus its own explicit live-verification gap (0012), the deduplication engine's confidence-threshold design plus the explainable lead-scoring algorithm (0013), the Lead Workspace's design — no lead-status state machine, assignment/tag/saved-view design, the dedup-merge Lead-reassignment fix, and the bulk-route-ordering fix (0014), the Exports design — the `Export` row as its own audit/status record, replayable (not frozen) selection, signed-URL-per-request storage, streaming XLSX generation, uniform formula-injection protection, and the never-fabricated Opening Hours/Verification Status columns (0015), the CRM push/CSV import design — the generic-webhook-not-named-connector decision, credential encryption, HMAC signing, SSRF-safety reuse, CSV import's deliberate non-implementation of `BaseConnector`, and the two systemic Celery retry-exhaustion bugs found and fixed across five task files (0016), the billing provider integration — real Stripe Checkout/Portal/webhook, the webhook-as-sole-source-of-truth decision, `SubscriptionPlan.stripe_price_id` as real never-fabricated per-deployment config, and the webhook route as the first HMAC-signature-authenticated (not session-authenticated) endpoint in this codebase (0017), the CI/CD pipeline plus RLS-coverage/route-shadowing hardening tests — both explicitly named "Milestone 10" gaps in ADR-0001/ADR-0007, verified to catch real regressions rather than pass vacuously (0018), the RLS ordering guard — an always-on `do_orm_execute`/`after_transaction_end` runtime check scoped to a dedicated `_AppSession` class, chosen over a static lint rule, plus the `roles`/`role_permissions` policy-exception and `audit_logs`/`platform_audit_logs` substring-collision bugs found and fixed while validating it (0019), and the shared Celery retry-exhaustion helper — `retry_or_finalize`, the one sanctioned way any of this worker's five retrying tasks checks retry-budget exhaustion, closing ADR-0016's own named follow-up risk, plus the `except ... as exc` closure-unbinding footgun found and fixed while migrating to it (0020), the recurring credit grant on billing renewal — every paid subscription invoice grants its plan's `monthly_credit_grant` (not just later renewals), resolved from the invoice's own line item rather than the separately-synced subscription row to avoid a webhook-delivery-order dependency, idempotent per Stripe invoice id via a locked `InvoiceRecord` column rather than relying on the outer per-event dedup alone (0021), and export storage cleanup — a real, disclosed `EXPORT_RETENTION_DAYS` default (not a fabricated requirement), an hourly Celery beat sweep mirroring `expire_stale_reservations`'s exact shape, idempotency keyed on a new `storage_deleted_at` column plus S3's own idempotent-delete guarantee, verified directly against a simulated crash window rather than only reasoned about, and the download endpoint now refusing to presign a URL for an already-cleaned-up export (0022), and connector fault injection plus two real retry-loop bug fixes — `FaultInjectingConnector` (test-only, never registered in production) drove `campaign_tasks`' real Celery retry loop and found a genuine bug (a transient failure left the task stuck, making every subsequent retry silently no-op instead of re-attempting the work), pattern-matching that shape across the other four retrying tasks found the identical bug in `enrichment_tasks` and confirmed the remaining three were already correctly designed against it, both bugs fixed by resetting the claimable-state field before a genuine retry, and a latent shared-DB-engine/redis cross-event-loop test-isolation gap found and fixed along the way (0023), and the enrichment trigger/evidence UI — a new card on the lead detail page over already-existing, unchanged-since-Milestone-4 backend endpoints, live-verified against the real running stack in a headless browser, plus the unrelated seeded-demo-tenant `.local`-email `EmailStr`-rejection bug found and disclosed (not fixed) during that verification (0024), and the duplicate-candidate review UI — Confirm/Reject buttons wired onto the existing Duplicate Candidates card plus an entirely new Merge History card with Undo, both resolving business names via a batched `useQueries` lookup since neither response schema carries one, live-verified against a genuinely-seeded `pending` fuzzy-name-tier candidate produced via a mock campaign plus a CSV import, using a standalone `moto` S3 server to satisfy CSV import's object-storage dependency in this Docker-less sandbox (0025), and the seeded demo tenant email domain fix — changing every demo account's placeholder domain from the `EmailStr`-rejected `.local` special-use TLD to RFC 2606's reserved `example.com` documentation domain, a seed-data-only change with no schema impact, live-verified with real logins for both the tenant-owner and platform-admin demo accounts (0026), and bulk lead scoring — a new `score_businesses_for_campaign` service function reusing `score_lead` in a loop under one commit, a bulk action rather than an automatic post-discovery trigger (deliberately the smaller of the two options the disclosed limitation named), a small DRY refactor extracting `ScoreLeadResponse.from_score_result` for both the single and bulk routes to share, and a single new "Score all businesses" button on the campaign detail page, live-verified against a real 8-business campaign (0027), and the OpenStreetMap/Overpass connector — a third, credential-free `BaseConnector` implementation registered as `osm`, a new `supports_rating_filter` interface flag that makes campaign creation reject rating/review filters for a source with no such data rather than silently ignoring them, an industry→OSM-tag mapping table, a frontend Data source selector (previously hardcoded to `mock`), and a live-verified honest failure path (a real launch against this sandbox's genuinely blocked Overpass access correctly retries then fails cleanly with full credit release) since the real network path itself remains unverified here, the same disclosed shape of gap as Google Places (0028).
...
4. **RLS discipline is now automatically checked for both *coverage* (Milestone 10, ADR-0018) and *ordering* (Milestone 11, ADR-0019), for ~44 of the ~46 RLS-protected tables.** `apps/api/tests/test_rls_coverage.py` verifies every tenant-owned table has real, working RLS. `app/core/db.py`'s runtime guard now additionally raises immediately, in every environment, if any query against one of those tables runs on a session that never called `set_tenant_context`/`set_platform_bypass` — closing ADR-0007's named ordering gap for the tables it covers. The residual exception: `roles` and `role_permissions` are deliberately excluded from the guard (their RLS policy has an extra `tenant_id IS NULL` clause allowing platform-role catalog rows to be read with no context — a real, correct design predating this milestone), so a context-less query against the *tenant-scoped* half of those two specific tables would still silently return zero rows, undetected by this guard. Milestone 2's worker code hit the original, now-mostly-closed version of this bug once (`worker.campaign_tasks._run_campaign_task_async`'s first lookup needed `set_platform_bypass` before its first read) before being fixed.
...
10. **Closed (Milestone 15, ADR-0023): no automated test exercised a real connector error path end-to-end through the worker's retry logic.** `FaultInjectingConnector` (`connector_sdk`, test-only, never registered in production) now drives `worker.campaign_tasks.run_campaign_task`'s real Celery retry loop with a scripted sequence of connector failures. Building this test found and fixed a real bug (see limitation #39): a transient failure left the campaign task stuck at `"running"`, so a genuine retried delivery silently no-op'd instead of ever re-attempting the connector call. The same investigation found and fixed an identical bug in `worker.enrichment_tasks`, and confirmed the other three retrying tasks (`export_tasks`, `csv_import_tasks`, `integration_tasks`) don't share it.
...
13. **Closed (Milestone 16, ADR-0024): no frontend UI existed for triggering enrichment or viewing evidence.** The lead detail page (`/leads/[id]`) now has a "Website enrichment" card — trigger, live status polling, and a rendered evidence list — calling the same `POST/GET /businesses/{id}/enrich(ment)`/`evidence` endpoints that have existed unchanged since Milestone 4. No backend/worker code changed; live-verified against the real running stack in a headless browser.
14. **Closed (Milestone 17, ADR-0025): Milestone 5's duplicate-candidate-review/merge/undo endpoints now have a frontend home.** The lead detail page's Duplicate Candidates card now has Confirm/Reject buttons for each `pending` candidate, and a new Merge History card lists confirmed merges with an Undo button — both resolving the "other business"'s real name via a batched `useQueries` lookup, since neither response schema includes one. No backend/worker code changed; live-verified against a real, genuinely-seeded `pending` fuzzy-match candidate (a mock campaign plus a CSV-imported near-duplicate name).
...
18. **Closed (Milestone 19, ADR-0027): scoring is no longer only manual, per-business.** `POST /campaigns/{campaign_id}/score-businesses` scores every canonical business a campaign discovered in one call, and a "Score all businesses" button on the campaign detail page triggers it. Deliberately built as an explicit bulk action, not an automatic post-discovery trigger — the latter would change every tenant's campaign behavior unconditionally with no way to opt out, a larger change than the disclosed gap asked for; it remains a candidate for a future, separately-scoped milestone if wanted.
...
23. **Closed (Milestone 14, ADR-0022): `Export.expires_at` is now consumed by a real cleanup job.** It previously marked when the underlying object *could* become eligible for storage cleanup, but nothing deleted expired export objects. `worker.export_cleanup_tasks.cleanup_expired_exports_task` (hourly Celery beat, `queue.maintenance`, mirroring `expire_stale_reservations`'s exact shape) now deletes each expired export's object and marks a new `Export.storage_deleted_at` column; the download endpoint refuses to presign a URL for an export whose storage has already been cleaned up. The retention period (`EXPORT_RETENTION_DAYS`, default 30) is a disclosed operational default, not a spec'd requirement — no captured requirement specified one.
...
28. **A systemic Celery retry-exhaustion bug (ADR-0016, Bug 2) existed in five task files since Milestones 2, 4, and 7, undetected until Milestone 8's live verification led to inspecting `Task.retry()`'s actual behavior.** `except MaxRetriesExceededError:` around a `retry(exc=exc, ...)` call never caught anything once retries were exhausted — every one of these tasks (`campaign_tasks`, `enrichment_tasks`, `export_tasks`, `csv_import_tasks`, `integration_tasks`) was silently dying uncaught on its final retry, with no error ever recorded, instead of cleanly finalizing as failed. Fixed by hand at the time in all five files; **as of Milestone 12 (ADR-0020), the fix itself is centralized in `worker/retry.py`'s `retry_or_finalize`** — the one sanctioned way any task in this codebase checks retry-budget exhaustion, so a *future* task literally cannot reintroduce the old broken shape by omission. It went unnoticed for three prior milestones because no automated test exercised a real Celery-wrapped task through actual retry exhaustion — still true today for each task's own domain-specific finalize behavior (still covered by that task's own test), but the exhaustion-vs-retry *control flow* itself is now covered once, directly, by `test_retry.py`, not five separate copies of hand-written logic trusted by inspection.
...
30. **Closed (Milestone 13, ADR-0021): recurring credit grant on the billing cycle.** `SubscriptionPlan.monthly_credit_grant` previously only granted credits once, at tenant creation - a real subscription renewing monthly never re-granted credits each period. `billing.services._grant_recurring_credits_for_invoice` now grants it on every paid subscription `invoice.paid` webhook event, idempotent per Stripe invoice id (a locked `InvoiceRecord.credit_grant_applied_at` column, not just the outer per-event dedup). Residual gap: only the invoice's *first* line item is read (limitation #35) - correct for every scenario this codebase's own Checkout Sessions produce today, but would need revisiting if a future plan or connector adds multiple concurrent line items to one invoice.
...
32. **The RLS ordering guard (Milestone 11, ADR-0019) only fires on code paths that are actually exercised.** It is a runtime check, not a static one — a query on a code path that is written but never executed by any real request, task, or test would never trip it. This is an inherent limitation of any runtime-only mechanism, accepted as the tradeoff for a check that catches every *real* occurrence immediately, in any environment, with no lint-time step to remember to run. It is also scoped specifically to `_AppSession`/`AsyncSessionLocal` — a future piece of code that constructs its own `async_sessionmaker` outside of `app.core.db` (there is currently none in production code) would silently bypass it.
33. **`places.googleapis.com` is now genuinely network-reachable from this session's egress policy, but this remains unexploited (Milestone 12).** Confirmed with a live call that returned a real, correctly-shaped `API_KEY_INVALID` error from Google's own servers — a change from every prior milestone's disclosed gap (limitation #8, ADR-0010). `api.stripe.com` and general internet hosts (e.g. `example.com`) are still rejected by the proxy with policy `403`s, so this does not extend to Stripe/webhook live verification. No Google Cloud API key exists in this environment, and none was fabricated to exploit this — the connectivity is disclosed here so a future session that has a real key available knows the network path is already open and doesn't need to re-discover this.
34. **The Celery retry-exhaustion control flow is now centralized (Milestone 12, ADR-0020), but each task's own domain-specific finalize behavior is not.** `retry_or_finalize` guarantees the exhaustion-check-before-`retry()` control flow can't be gotten wrong by a future task; it does not (and structurally cannot) guarantee that a given task's `finalize` coroutine itself correctly marks the right record as failed with a useful error — that remains each task's own responsibility and each task's own test's own job to verify, exactly as before this milestone.
35. **The recurring credit grant (Milestone 13, ADR-0021) only reads an invoice's first line item.** `_grant_recurring_credits_for_invoice` resolves the plan from `invoice["lines"]["data"][0]["price"]["id"]` - correct for every invoice shape this codebase's own Checkout Sessions produce today (a single-price subscription, no add-on line items), but a future invoice with multiple concurrent line items (e.g. the subscription's own line plus a manually-added one-off invoice item) would need this changed to find the *subscription's* line item specifically, not just the first one in the list.
36. **The recurring credit grant has never been exercised against a real Stripe-delivered `invoice.paid` event (Milestone 13, ADR-0021) — the same shape of gap as the rest of the billing module (limitation #29).** The signature-verification path and the grant/idempotency logic downstream of it are both real and tested against genuinely-signed synthetic payloads (including a genuine redelivered-invoice-under-a-different-event-id scenario); Stripe's own actual invoice/line-item payload shape for every real billing scenario (proration, multiple line items, a manually-added invoice item) has not been observed.
37. **The export retention period is a single global setting, not configurable per tenant or per export (Milestone 14, ADR-0022).** `EXPORT_RETENTION_DAYS` applies uniformly to every export in this deployment. Nothing in the captured requirements asked for per-tenant retention policies, so none was invented; a future billing tier wanting a longer or shorter retention window would need this revisited.
38. **The export storage cleanup sweep has been verified against `moto`'s real local S3-API server, not a real MinIO/S3 deployment (Milestone 14, ADR-0022) — the same shape of gap as the rest of object storage (limitation #6).** `DeleteObject`'s idempotent-against-an-already-absent-key behavior is standard S3 protocol semantics `moto` implements faithfully, but a real deployment's own quirks have not been observed for this specific operation.
39. **Closed (Milestone 15, ADR-0023): two real retry-loop bugs in `worker.campaign_tasks` and `worker.enrichment_tasks`, present since Milestone 2/4 respectively, undetected until this milestone's connector-fault-injection test finally drove each real Celery-wrapped task through a genuine retry.** Both tasks committed a claimable-state field (`CampaignTask.status`/`BusinessEnrichment.status`) to `"running"` before their own retryable failure point, and neither ever reset it back to `"pending"` before scheduling a real retry - so a genuine retried delivery hit the task's own duplicate-delivery guard and silently no-op'd instead of ever re-attempting the work, burning the entire retry budget doing nothing before eventually, correctly, finalizing as failed. No data was ever corrupted and no campaign/enrichment ever got permanently stuck - the terminal outcome was always eventually correct - but the retry-then-recover mechanism itself never functioned for either task until this fix. `export_tasks`, `csv_import_tasks`, and `integration_tasks` were checked and confirmed not to share this bug (two by deliberate, already-documented design; one by never committing an intermediate status change before its retry point).
40. **A pre-existing worker test-infrastructure gap was found and fixed alongside the above (Milestone 15, ADR-0023): the production DB engine/redis client could accumulate connections bound to the wrong event loop across different testing styles in the same test session.** Ordinary tests calling a task's `_run_..._async` coroutine directly share pytest-asyncio's session-scoped loop; a test driving the real Celery-wrapped task via `run_db_task` opens a fresh loop per call and, without the new `real_celery_task_isolation` fixture (`apps/worker/tests/conftest.py`), would try to close connections left behind by the former from the latter's unrelated loop. Any *future* test file adopting the real-Celery-wrapped-task pattern must request this fixture, or it may intermittently fail depending on which other test files happen to run first in the same session.
41. **Closed (Milestone 18, ADR-0026): the seeded demo tenant's email addresses now log in through the real API.** Every demo-tenant email address (`app/seed/seed_data.py`'s `DEMO_TENANT_USERS`/`PLATFORM_DEMO_USER`) changed from the `EmailStr`-rejected `.local` special-use TLD to RFC 2606's reserved `example.com` documentation domain (e.g. `owner@northstar-demo.gridkeep-demo.example.com`) — a seed-data-only change, no schema/migration impact. Live-verified with real `POST /auth/login` requests for both the tenant-owner and platform-admin demo accounts, both returning `200` with a real session cookie. A database seeded before this fix retains its old `.local` rows as harmless, unreachable leftovers (the seed script's idempotency is keyed by email, so re-seeding adds the new rows rather than renaming the old ones).
42. **Manually starting the Celery worker for live verification needs every queue named explicitly (found during Milestone 17's live-verification pass, ADR-0025).** `worker/celery_app.py` routes tasks across five named queues (`queue.search`, `queue.enrichment`, `queue.export`, `queue.crm_push`, `queue.maintenance`), not Celery's default queue. A worker started with no `-Q` flag accepts campaign/CSV-import tasks onto its connection but never executes them — the campaign/import sits stuck at `queued`/`processing` with no error anywhere, since nothing failed, the task was simply never delivered to a consumer listening on the right queue. Not a code bug (every production entry point that starts this worker for real already specifies its queues correctly); purely a manual-verification gotcha worth recording so a future milestone's live pass doesn't lose time rediscovering it. Milestone 17's verification also needed a real S3-compatible endpoint for CSV import's synchronous upload step (limitation #6's same gap): a standalone `moto.server.ThreadedMotoServer`, matching the test suite's own `moto_s3` fixture, stood in.
43. **`osm` (the OpenStreetMap/Overpass connector) has never made a real call to the Overpass API (Milestone 20, ADR-0028) — the same shape of gap as Google Places (#8).** This sandbox's egress proxy rejects every Overpass/OpenStreetMap host tried (`overpass-api.de`, `overpass.kumi.systems`, `nominatim.openstreetmap.org`) with a policy-level 403, confirmed directly with `curl` before any connector code was written. The connector is built against Overpass's documented QL syntax and real JSON response shape, covered by 23 mocked-HTTP-transport tests, and its *failure* path has been live-verified (a real launch against the blocked network correctly retries then fails cleanly with full credit release) — but its real-query *success* path (actual parsing of Overpass's real response for a real city+category) has not been observed. **Before relying on this connector for real leads**: run one real `search()` call by hand from an environment without this restriction and inspect the result.
44. **The OpenStreetMap connector has several disclosed, by-design scope limits (Milestone 20, ADR-0028).** `region`/`area` campaign filters and `radius_km` are not applied to the Overpass query itself for this source (only country+city scope the search) — `CampaignFilter` has no lat/lon fields, and nesting a third level of OSM area matching reliably was judged not worth the complexity for a first version. An industry/category string outside the ~30-entry `INDUSTRY_TAG_MAP` is rejected with a clear error rather than guessed at. A multi-page campaign against this source re-queries Overpass once per page rather than once total (no result cache, to avoid cross-tenant staleness/memory-growth concerns in the shared connector-registry singleton) — real added network cost for very large result limits, though Overpass's own queries are typically fast for a single city+category.
...
- **Connection-pool GUC leakage class of bug** (ADR-0007): both the *coverage* half (Milestone 10's `test_rls_coverage.py`) and, as of this milestone, the *ordering* half are now automated. `app/core/db.py`'s runtime guard (ADR-0019) raises immediately if a query against an RLS-protected table runs on a session lacking a context marker — closing the specific risk that hit Milestone 2's worker code once already (see limitation #4). The residual, narrower risk: `roles`/`role_permissions` are deliberately excluded from the guard (their policy's extra `tenant_id IS NULL` clause for platform-role catalog rows means the guard can't distinguish a legitimately-context-less read from a genuinely missing one on those two tables specifically) — a context-less query against the *tenant-scoped* half of either table would still silently return zero rows, undetected. Closing this fully would require real per-query SQL-clause parsing, not just knowing which table is being queried.
...
- **Closed (limitation #10, Milestone 15, ADR-0023): no connector-error-path test coverage through the worker's own retry loop.** `FaultInjectingConnector` now drives `run_campaign_task`'s retry/backoff/permanent-failure handling end-to-end with an injected failure - and doing so found and fixed two real bugs (limitation #39): `campaign_tasks` and `enrichment_tasks` both left their claimable-state field `"running"` across a retryable failure, making every genuine retry silently no-op. Also surfaced and fixed a latent worker test-infrastructure gap (limitation #40): a shared DB engine/redis client accumulating connections bound to the wrong event loop across different testing styles in one session.
- **Closed (limitation #41, Milestone 18, ADR-0026): the seeded demo tenant could not log in through the real API.** Fixed by changing every demo email address's domain from the `EmailStr`-rejected `.local` special-use TLD to RFC 2606's reserved `example.com` documentation domain - a seed-data-only change, live-verified with real logins for both the tenant-owner and platform-admin demo accounts.
- **Closed (limitation #14, Milestone 17, ADR-0025): the duplicate-candidate review/merge/undo endpoints had no frontend home.** Confirm/Reject buttons are now wired onto the lead detail page's Duplicate Candidates card, and a new Merge History card surfaces Undo — both resolving business names via a batched lookup, live-verified against a real, genuinely-seeded `pending` fuzzy-match candidate rather than fabricated data.
...
- **Closed (limitation #23, Milestone 14, ADR-0022): no storage-cleanup job existed for completed exports' underlying objects.** An hourly Celery beat sweep now deletes each expired export's object and marks `storage_deleted_at`, idempotent against a crash between the delete and the mark (proven by a dedicated test, not just reasoned about). Residual: the retention period is a single global setting (limitation #37), and this has only been verified against `moto`'s real local S3-API server, not a real MinIO/S3 deployment (limitation #38) — the same shape of gap as the rest of object storage.
...
- **The systemic Celery retry-exhaustion bug (limitation #28) was only found by accident, three milestones after the earliest instance was introduced — now closed (Milestone 12, ADR-0020).** Its root cause (Celery's `retry(exc=exc, ...)` re-raising the original exception instead of `MaxRetriesExceededError` once retries are exhausted) was fixed by hand everywhere it existed at the time (Milestone 8); as of this milestone, the fix itself is centralized in `worker/retry.py`'s `retry_or_finalize` — the one sanctioned way any task in this codebase checks retry-budget exhaustion, so a *future* task can no longer reintroduce the old broken shape by omission the way the original bug happened. The residual risk (limitation #34): `retry_or_finalize` only guarantees the exhaustion-vs-retry *control flow*, not that any given task's own `finalize` coroutine correctly marks the right record failed with a useful error — that remains each task's own responsibility, same as before.
- **Google Places is now network-reachable from this session (limitation #33, Milestone 12) but still unverified live, since no API key exists here.** The connectivity gap ADR-0010 originally disclosed is now confirmed closed at the network level; the credential gap is not. A future session with a real Google Cloud API key can now run the live smoke test ADR-0010 always recommended without first needing to solve a network-access problem.
...
- **Closed (limitation #30, Milestone 13, ADR-0021): recurring credit grant on subscription renewal.** Every paid subscription invoice now grants its plan's `monthly_credit_grant`, idempotent per Stripe invoice id (proven against a genuine redelivered-under-a-different-event-id scenario, not just declared). Residual: only the invoice's first line item is read (limitation #35), and this has never been exercised against a real Stripe-delivered event (limitation #36) — the same shape of gap as the rest of the billing module.
...
- **The RLS *ordering* rule now has an automated runtime guard for ~44 of ~46 RLS-protected tables (limitation #4, closed Milestone 11, ADR-0019).** `app/core/db.py`'s `do_orm_execute`/`after_transaction_end` listeners raise immediately, in every environment, if a query against a guarded table runs on a session lacking a context marker — this is the risk ADR-0007 originally flagged, and Milestone 2 proved real once. The residual, much narrower risk (limitation #32): `roles`/`role_permissions` are excluded from the guard for a genuine, documented policy-shape reason, so a context-less query against their tenant-scoped half specifically would still go undetected; and the guard is inherently runtime-only, so a written-but-never-executed code path would never trip it.
- **The RLS ordering guard is scoped to `_AppSession`/`AsyncSessionLocal` only (Milestone 11, ADR-0019, limitation #32).** Confirmed via `grep -rln "async_sessionmaker("` that this is currently the only session factory in this codebase's production code, so the guard covers every real request/task path today — but a future piece of code that builds its own `async_sessionmaker` outside `app.core.db` would silently bypass it, since the guard is attached to a specific `Session` subclass, not globally.
- **The OpenStreetMap/Overpass connector has never been called live (limitation #43, Milestone 20, ADR-0028)** — the same shape of gap as Google Places (#8) and the SSRF crawler (#12), caused here by this sandbox's egress proxy rejecting every Overpass/OpenStreetMap host tried. Its failure path is live-proven (a real launch against the blocked network retries then fails cleanly with full credit release); its real-query success path against actual Overpass data has not been observed. Treat it as unverified until run from an environment without this restriction.

## Pending approvals
None outstanding. Milestone 20's scope (the OpenStreetMap/Overpass connector) came directly from the user's own request, not self-selection — the first milestone since 10 to work this way. ADR-0028 documents the full design — the `supports_rating_filter` interface addition and why rating filters are rejected rather than silently ignored, the industry→OSM-tag mapping approach, the country→city area-chaining and no-server-side-pagination tradeoffs, and the live-verified honest-failure-path verification (proving the retry/credit-release machinery works correctly for a brand-new connector even though the real Overpass network path itself remains unverified in this sandbox) — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 20. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 21 started next. No spec text in this session names Milestone 21's scope. Most remaining open items are either deliberate by-design tradeoffs or blocked on external credentials/infrastructure this sandbox does not have (a real Google Cloud API key, a real Stripe test-mode key, general internet egress, a real MinIO/Docker deployment) — the OpenStreetMap connector itself now carries the same "built and mock-tested, not live-verified" gap Google Places has (limitation #43), closable only from an environment with unrestricted egress, not more code. If you have any of those credentials/environments available, that would unblock the strongest remaining candidate. Otherwise, further progress needs new scope from you.
...
## Complete file inventory (created or modified in Milestone 11)

### Backend — `apps/api`
`app/core/db.py` (modified — new `_AppSession` class, `_RLS_CONTEXT_MARKER`, `_GUARD_EXCLUDED_TABLES`, `_rls_protected_table_names`/`_rls_protected_table_pattern` helpers, `_guard_rls_context_before_execute`/`_clear_rls_context_marker_on_outer_transaction_end` event listeners, `AsyncSessionLocal` built with `sync_session_class=_AppSession`; `set_tenant_context`/`set_platform_bypass` now set the `session.info` marker),
`tests/test_rls_ordering_guard.py` (new, 9 tests)

### Docs
`docs/project-status.md`, `docs/adr/0019-rls-ordering-guard.md` (new)

---

## Complete file inventory (created or modified in Milestone 12)

### Worker — `apps/worker`
`worker/retry.py` (new — `retry_or_finalize`, `default_backoff`),
`worker/campaign_tasks.py` (modified — both retry branches migrated to `retry_or_finalize`; removed now-unused `logger`/`get_logger`),
`worker/enrichment_tasks.py`, `worker/export_tasks.py`, `worker/csv_import_tasks.py`, `worker/integration_tasks.py` (each modified — retry-exhaustion `if`/`else` block replaced by a `retry_or_finalize` call),
`tests/test_retry.py` (new, 5 tests),
`tests/test_integration_tasks.py` (modified — one test's `run_db_task` monkeypatch target updated to also patch `worker.retry.run_db_task`)

### Docs
`docs/project-status.md`, `docs/adr/0020-shared-celery-retry-exhaustion-helper.md` (new)

---

## Complete file inventory (created or modified in Milestone 13)

### Backend — `apps/api`
`alembic/versions/7c27453a6ffb_add_invoice_records_credit_grant_.py` (new — adds `invoice_records.credit_grant_applied_at`),
`app/modules/billing/models.py` (modified — `InvoiceRecord.credit_grant_applied_at`),
`app/modules/billing/repositories.py` (modified — `get_invoice_by_stripe_id_for_update`),
`app/modules/billing/services.py` (modified — `_grant_recurring_credits_for_invoice`, wired into `_handle_invoice_event`),
`tests/test_billing_api.py` (modified — 3 new tests plus shared helpers: `_wallet_balance`, `_credit_grant_count_for_reference`, `_invoice_paid_event`, `_post_signed_webhook`)

### Docs
`docs/project-status.md`, `docs/adr/0021-recurring-credit-grant-on-billing-renewal.md` (new)

---

## Complete file inventory (created or modified in Milestone 14)

### Backend — `apps/api`
`alembic/versions/17cc4debecf6_add_exports_storage_deleted_at.py` (new — adds `exports.storage_deleted_at`),
`app/core/config.py` (modified — `export_retention_days`),
`app/core/storage.py` (modified — `delete_object`),
`app/modules/exports/models.py` (modified — `Export.storage_deleted_at`),
`app/modules/exports/repositories.py` (modified — `list_expired_uncleaned_exports`, `mark_storage_deleted`),
`app/modules/exports/services.py` (modified — `cleanup_expired_exports`, `get_download_url`'s new storage-deleted check),
`tests/test_exports.py` (modified — 1 new test)

### Worker — `apps/worker`
`worker/export_cleanup_tasks.py` (new),
`worker/export_tasks.py` (modified — real `expires_at` on completion),
`worker/celery_app.py` (modified — task registration/routing),
`worker/beat_schedule.py` (modified — hourly schedule entry),
`tests/test_export_cleanup_tasks.py` (new, 4 tests)

### Docs
`docs/project-status.md`, `docs/adr/0022-export-storage-cleanup.md` (new)

---

## Complete file inventory (created or modified in Milestone 15)

### Connector SDK — `packages/connector-sdk`
`connector_sdk/fault_injecting.py` (new — `FaultInjectingConnector`, test-only, not exported/registered),
`tests/test_fault_injecting.py` (new, 4 tests)

### Worker — `apps/worker`
`worker/campaign_tasks.py` (modified — reset-to-pending fix, `_reset_task_to_pending_for_retry`),
`worker/enrichment_tasks.py` (modified — the identical fix, `_reset_enrichment_to_pending_for_retry`),
`tests/test_campaign_task_retry_loop.py` (new, 3 tests),
`tests/test_enrichment_task_retry_loop.py` (new, 1 test),
`tests/conftest.py` (modified — new `real_celery_task_isolation` fixture)

### Docs
`docs/project-status.md`, `docs/adr/0023-campaign-task-retry-loop-fix.md` (new)

---

## Complete file inventory (created or modified in Milestone 16)

### Frontend — `apps/web`
`lib/types.ts` (modified — `Enrichment`, `EnrichmentEvidence`, `EnrichmentStatus`),
`app/(tenant)/leads/[id]/page.tsx` (modified — new "Website enrichment" card, queries, trigger handler)

### Docs
`docs/project-status.md`, `docs/adr/0024-enrichment-frontend.md` (new)

---

## Complete file inventory (created or modified in Milestone 17)

### Frontend — `apps/web`
`lib/types.ts` (modified — new `MergeHistoryEntry` type),
`app/(tenant)/leads/[id]/page.tsx` (modified — merge-history query, batched business-name `useQueries` lookup, `handleConfirmCandidate`/`handleRejectCandidate`/`handleUndoMerge` handlers, updated Duplicate Candidates card, new Merge History card)

### Docs
`docs/project-status.md`, `docs/adr/0025-duplicate-review-frontend.md` (new)

---

## Complete file inventory (created or modified in Milestone 18)

### Backend — `apps/api`
`app/seed/seed_data.py` (modified — `DEMO_TENANT_USERS`/`PLATFORM_DEMO_USER` domain changed from `.local` to `example.com`)

### Docs
`docs/project-status.md`, `docs/adr/0026-seed-demo-tenant-email-domain-fix.md` (new)

---

## Complete file inventory (created or modified in Milestone 19)

### Backend — `apps/api`
`app/modules/leads/scoring.py` (modified — new `score_businesses_for_campaign` function),
`app/modules/leads/schemas.py` (modified — new `ScoreLeadResponse.from_score_result` classmethod),
`app/modules/businesses/routes.py` (modified — `score_business` refactored to use the new classmethod; unused imports removed),
`app/modules/campaigns/routes.py` (modified — new `POST /{campaign_id}/score-businesses` route)

### Backend tests — `apps/api`
`tests/test_lead_scoring.py` (modified — 2 new tests for `score_businesses_for_campaign`),
`tests/test_campaign_engine.py` (modified — 1 new HTTP-level permission/routing test)

### Frontend — `apps/web`
`lib/types.ts` (modified — new `ScoreLeadResult` type),
`app/(tenant)/campaigns/[id]/page.tsx` (modified — new "Score all businesses" button, handler, and success banner)

### Docs
`docs/project-status.md`, `docs/adr/0027-bulk-lead-scoring.md` (new)

---

## Complete file inventory (created or modified in Milestone 20)

### Connector SDK — `packages/connector-sdk`
`connector_sdk/overpass.py` (new — `OverpassConnector`),
`connector_sdk/base.py` (modified — new `supports_rating_filter` attribute),
`connector_sdk/registry.py` (modified — `osm` registration),
`connector_sdk/__init__.py` (modified — `OverpassConnector` export),
`tests/test_overpass.py` (new, 23 tests)

### Backend — `apps/api`
`app/modules/campaigns/services.py` (modified — `create_campaign` rejects `min_rating`/`min_reviews` for a connector with `supports_rating_filter = False`)

### Backend tests — `apps/api`
`tests/test_campaign_engine.py` (modified — 2 new tests: rating-filter rejection, successful osm-sourced creation)

### Frontend — `apps/web`
`app/(tenant)/campaigns/page.tsx` (modified — Data source selector, per-source caption, matching client-side zod validation)

### Docs
`docs/project-status.md`, `docs/adr/0028-openstreetmap-overpass-connector.md` (new)
