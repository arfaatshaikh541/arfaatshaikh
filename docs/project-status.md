...
## Current milestone
**Milestone 15: connector fault injection and two real retry-loop bug fixes** — implementation complete, backend-only (this milestone has no frontend surface). No spec text named Milestone 15 directly; chosen as the next highest-value credential-free gap under the user's "Milestone 15" authorization. Closes a testing gap named since Milestone 3: "no automated test exercises a real connector error path end-to-end through the worker's retry logic." Delivered a new `FaultInjectingConnector` (`connector_sdk`, test-only, never registered in production) to drive `worker.campaign_tasks.run_campaign_task`'s real Celery-wrapped retry loop with a scripted, deterministic sequence of connector failures — and building that test surfaced a genuine, previously-undetected production bug: a transient connector failure left the campaign task stuck at `"running"`, so a real retried delivery silently no-op'd against its own duplicate-delivery guard instead of ever re-attempting the connector call, burning through the entire retry budget doing nothing before eventually finalizing as failed. Checking the other four retrying tasks for the same shape found an identical bug in `worker.enrichment_tasks` (confirmed with its own dedicated test before fixing) — and confirmed the other three (`export_tasks`, `csv_import_tasks`, `integration_tasks`) were already correctly designed against it, two of them deliberately and with in-code comments saying so. Both bugs are now fixed with the same pattern: reset the claimable-state field back to `pending` before a genuine retry, never before finalizing as failed. A pre-existing, latent test-infrastructure gap (a shared DB engine/redis client accumulating connections bound to the wrong event loop across different testing styles in the same session) was also found and fixed with a new shared `real_celery_task_isolation` fixture. See ADR-0023. Pending your review before proceeding to Milestone 16.
...
## Pending approvals
None outstanding. Milestone 15's scope (connector fault injection) was self-selected under the user's bare "Milestone 15" authorization as the next highest-value credential-free gap — a real, long-standing, explicitly-named testing gap (#10, carried since Milestone 3). Building the test this gap asked for surfaced two genuine production bugs (in `campaign_tasks` and `enrichment_tasks`) and a latent test-infrastructure gap, none of which were anticipated when the milestone was chosen — all found, verified, and fixed within the same milestone rather than deferred, matching this project's own precedent (ADR-0016 fixed all five instances of its own systemic bug in one milestone rather than one at a time). ADR-0023 documents the full investigation — what was found, how each of the other three tasks was checked and confirmed not to share the bug, and why the fix lives locally in each affected task rather than in the shared `retry_or_finalize` helper — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 15. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 16 started next. No spec text in this session names Milestone 16's scope, the same situation every milestone since 10 has been in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value credential-free gap to close.
