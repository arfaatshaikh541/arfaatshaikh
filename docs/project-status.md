...
## Current milestone
**Milestone 13: recurring credit grant on billing renewal** — implementation complete, backend-only (this milestone has no frontend surface). No spec text named Milestone 13 directly; chosen as the next highest-value credential-free gap under the user's "Milestone 13" authorization. Closes a known limitation carried since Milestone 9: `SubscriptionPlan.monthly_credit_grant` was only ever applied once, at tenant creation for the trial plan — a real subscription renewing monthly never re-granted credits each billing period. Delivered: `billing.services._grant_recurring_credits_for_invoice`, run on every `invoice.paid` webhook event for a subscription invoice, grants the invoiced price's mapped plan's `monthly_credit_grant` — every paid subscription invoice, not just later renewals, since a tenant's Milestone-1 initial grant is for the *trial* plan specifically, not whatever plan they actually check out into. Idempotent per Stripe invoice id (a new locked `InvoiceRecord.credit_grant_applied_at` column), not just per Stripe event id — the outer `BillingEvent`-id dedup alone has a crash-window gap that a genuinely redelivered event under a different event id (which Stripe's own docs warn can happen) would slip through. This exact scenario — two distinct, genuinely-signed webhook deliveries for the same invoice under two different event ids — is proven, not just reasoned about, by a new test against the real database. See ADR-0021. Pending your review before proceeding to Milestone 14.
...
| Backend tests pass | ✅ 153/153 (api) + 77/77 (worker) + 22/22 (connector-sdk) = 252/252 |
...
## Pending approvals
None outstanding. Milestone 13's scope (recurring credit grant on billing renewal) was self-selected under the user's bare "Milestone 13" authorization as the next highest-value credential-free gap — a real, long-standing, explicitly-named limitation (#30, carried since Milestone 9) that needed no live Stripe account to implement or test for real, since the webhook *receiver* has always been testable with genuinely-signed synthetic payloads (ADR-0017). ADR-0021 documents the full design — the every-paid-invoice-not-just-renewals decision, resolving the plan from the invoice's own line item rather than the separately-synced subscription row to avoid a webhook-delivery-order dependency, and the invoice-id-not-just-event-id idempotency design (proven against a genuine redelivered-under-a-different-event-id scenario) — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 13. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 14 started next. No spec text in this session names Milestone 14's scope, the same situation every milestone since 10 has been in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value credential-free gap to close.
