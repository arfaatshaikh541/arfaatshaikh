...
## Current milestone
**Milestone 16: enrichment trigger and evidence UI** — implementation complete, frontend-only. No spec text named Milestone 16 directly; chosen as the next highest-value gap under the user's "Milestone 16" authorization — unlike Milestones 11-15 (all backend/worker), this one closes a frontend gap: website enrichment (crawl → detectors → evidence, opportunity detection) has been fully built and tested on the backend since Milestone 4, but had no UI surface at all, a known limitation left open for fifteen further milestones. Delivered a new "Website enrichment" card on the lead detail page (`/leads/[id]`) — an "Enrich website" trigger, live status polling (pending/running/completed/failed, the same `refetchInterval` pattern the campaign progress panel already established), and a rendered evidence list (detector type, source URL, confidence, supporting snippet) — calling the same `POST/GET /businesses/{id}/enrich(ment)`/`evidence` endpoints that have existed, unchanged, since Milestone 4. No backend or worker code was touched. Live-verified against the real running stack in a headless browser per the standing rule for UI changes: registered a user, launched a real mock campaign, scored a discovered business into a lead, and watched the enrich action genuinely poll from no-status through completed with a real evidence row rendered. That verification also surfaced an unrelated, real, disclosed bug (not fixed this milestone): the seeded demo tenant's `.local` email addresses fail `EmailStr` validation on the real login endpoint, so the demo accounts have apparently never been able to log in through the real API in this project's history. See ADR-0024. Pending your review before proceeding to Milestone 17.
...
## Pending approvals
None outstanding. Milestone 16's scope (the enrichment trigger/evidence UI) was self-selected under the user's bare "Milestone 16" authorization — deliberately a frontend gap this time, unlike Milestones 11-15, since website enrichment's fully-built, fully-tested backend capability (Milestone 4) had sat with zero UI surface for fifteen further milestones, more idle product value than any remaining backend gap offered. ADR-0024 documents the full design — the one-card-on-an-existing-page decision, the live-verification pass against the real running stack, and the unrelated seeded-demo-tenant login bug found and disclosed (not fixed) along the way — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 16. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 17 started next. No spec text in this session names Milestone 17's scope, the same situation every milestone since 10 has been in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value gap to close — the seeded-demo-tenant login bug (limitation #41) or the duplicate-candidate review buttons (limitation #14) are both small, well-scoped, credential-free candidates.
