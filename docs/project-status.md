...
## Current milestone
**Milestone 12: shared Celery retry-exhaustion helper** — implementation complete, backend-only (this milestone has no frontend surface). No spec text named Milestone 12 directly. Before implementation started, a real environment change was found and reported: `places.googleapis.com` is now genuinely network-reachable from this session (confirmed with a live call that returned a real, correctly-shaped `API_KEY_INVALID` error from Google's own servers), unlike every prior milestone. A live-verification pass was the standing recommendation for "next highest-value gap," but it still needs a real Google Cloud API key that isn't available in this environment and won't be fabricated — `api.stripe.com` and general internet hosts remain blocked regardless. With that path not actionable, this milestone instead closes ADR-0016's own explicitly-named follow-up risk: "nothing automated stops a future task from reintroducing the old `try/except MaxRetriesExceededError` shape" that caused three Celery tasks to silently die uncaught on real retry exhaustion, undetected across three prior milestones. Delivered: `apps/worker/worker/retry.py`'s `retry_or_finalize` — a shared function, not a lint rule, that is now the *only* way any of this worker's five retrying tasks (`campaign_tasks`, `enrichment_tasks`, `export_tasks`, `csv_import_tasks`, `integration_tasks`) checks retry-budget exhaustion before calling `Task.retry()`, replacing five copies of the identical hand-written `if self.request.retries >= self.max_retries` check with one. Migrating surfaced two real, fixed issues: a genuine Python semantic (`except ... as exc` unbinds `exc` when the block exits, breaking a naive deferred-closure `lambda`, caught correctly by ruff's F821) and one test whose `run_db_task` monkeypatch target moved with the refactor. See ADR-0020. Pending your review before proceeding to Milestone 13.
...
| Backend tests pass | ✅ 150/150 (api) + 77/77 (worker) + 22/22 (connector-sdk) = 249/249 |
...
## Pending approvals
None outstanding. Milestone 12's scope was chosen under the user's explicit "use default" instruction after an `AskUserQuestion` call attempting to confirm the choice failed to return a response (a tool-level error, not a user decline). The originally-stated default — a live-verification pass against real Google Places/Stripe/webhook credentials — was checked first and found not actionable: no API keys exist in this environment and none were fabricated, even though Google Places' network path is now open (see the Milestone 12 section above). The next-highest-value credential-free item was chosen instead: closing ADR-0016's own explicitly-named follow-up risk (a shared Celery retry-exhaustion helper). ADR-0020 documents the full design — the shared-function-not-lint-rule decision, the migration of all five tasks, and the two issues found and fixed while migrating (the `except ... as exc` closure-unbinding footgun, and the stale `run_db_task` monkeypatch target) — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 12. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 13 started next. No spec text in this session names Milestone 13's scope, the same situation Milestones 10-12 were all in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value credential-free gap to close.
