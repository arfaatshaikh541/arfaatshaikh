...
## Current milestone
**Milestone 17: duplicate-candidate review UI** — implementation complete, frontend-only. No spec text named Milestone 17 directly; chosen as "the other frontend gap flagged at the end of Milestone 16" under the user's bare "Milestone 17" authorization — Milestone 5/6's fully-built, fully-tested duplicate-candidate confirm/reject/undo endpoints had no frontend home, a known limitation left open for eleven further milestones. Wired "Confirm merge"/"Reject" buttons onto the lead detail page's existing Duplicate Candidates card and added an entirely new Merge History card (nothing previously rendered merge history in the frontend at all), each row resolving the "other business"'s name via a batched `useQueries` lookup against the existing `GET /businesses/{id}` endpoint (since neither response schema includes a name, only IDs). No backend or worker code was touched — every endpoint this UI calls has existed unchanged since Milestone 5/6. Live-verified against the real running stack in a headless browser: seeded a real `pending` fuzzy-name-tier duplicate candidate via a mock campaign plus a CSV import of a near-duplicate business name (standing up a standalone `moto` S3 server for CSV import's object-storage dependency, the same workaround the test suite's own `moto_s3` fixture already uses in this sandbox), then clicked Confirm on the actual page, watched the new Merge History card appear with the correct resolved names, clicked Undo, and watched it flip to "undone." See ADR-0025. Pending your review before proceeding to Milestone 18.
...
41. **The seeded demo tenant's email addresses cannot log in through the real API (found during Milestone 16's live-verification pass, ADR-0024, not yet fixed).** Every demo-tenant email address seeded since Milestone 1 (`app/seed/seed_data.py`'s `DEMO_TENANT_USERS`/`PLATFORM_DEMO_USER`) uses a `.local` TLD (e.g. `owner@northstar-demo.gridkeep.local`); Pydantic's `EmailStr` (via `email-validator`) rejects `.local` as a reserved, special-use TLD at the request-validation layer, before any service code runs, so `POST /auth/login` for any of these accounts always fails with a generic `422`. Every prior milestone's own live-verification pass registered a fresh `@example.com`-style user through the real signup flow instead of using the seeded demo accounts, which is presumably why this went unnoticed for fifteen milestones. A real fix (picking a different placeholder domain scheme, e.g. `*.example.com` subdomains, which `EmailStr` accepts) is a small, self-contained follow-up, deliberately not folded into Milestone 16 since it's unrelated to that milestone's actual scope.
...
- **The seeded demo tenant cannot log in through the real API (limitation #41, found during Milestone 16's live-verification pass, ADR-0024, not yet fixed).** Every demo-tenant email address uses a `.local` TLD, which `EmailStr` rejects as a reserved, special-use domain - `POST /auth/login` for any seeded demo account always 422s. Undetected for fifteen prior milestones because every live-verification pass registered a fresh `@example.com`-style user instead of using the seeded demo accounts. A small, self-contained fix (change the seed data's placeholder domain scheme) is recommended follow-up work.
...
## Pending approvals
None outstanding. Milestone 17's scope (the duplicate-candidate review UI) was self-selected under the user's bare "Milestone 17" authorization — "the other frontend gap flagged at the end of Milestone 16," the same reasoning that chose Milestone 16 itself: a fully-built, fully-tested backend capability (Milestone 5/6's confirm/reject/undo endpoints) that had sat with no UI surface for eleven further milestones. ADR-0025 documents the full design — wiring buttons onto the existing card plus a new Merge History card (rather than only the former), the batched business-name-resolution approach, the reactive-`business_id`-invalidation reasoning, and the live-verification pass (including the `moto` S3 workaround and the Celery queue-routing gotcha it surfaced) — and is called out here for transparency, same pattern as prior milestones' ADRs.

## Next action
Awaiting your review of Milestone 17. When ready, let me know how you'd like to proceed — including whether you'd like Milestone 18 started next. No spec text in this session names Milestone 18's scope, the same situation every milestone since 10 has been in. If you have a real Google Cloud API key (Places API (New) enabled, billing configured) and/or a real Stripe test-mode secret key, providing either would unblock the strongest remaining candidate — a genuine live-verification pass, now that Google Places' network path is confirmed open (Stripe/general internet egress remains blocked regardless of credentials). Otherwise I'd recommend reviewing the accumulated "Known limitations"/"Unresolved risks" lists for the next highest-value gap to close — the seeded-demo-tenant login bug (limitation #41) is now the most clearly-scoped, credential-free remaining candidate.
