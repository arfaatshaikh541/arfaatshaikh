...
| "close your ears" / "stop listening" / "shut down" commands | Partial | `VoiceSessionController.Sleep()` exists and is tested | Real | Yes | No | Nothing routes a recognized phrase like "close your ears" to `Sleep()` — STT output goes straight to the reasoning call, not to a command-phrase check first. (Corrected: an earlier draft of this row said "built this pass," contradicting this document's own closing summary — it was not, at the time this document was first written. See project-status.md / later commits for whether it has been since.) | `IMPLEMENTABLE_NOW` |
