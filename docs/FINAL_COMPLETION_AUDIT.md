...
|---|---|---|---|---|---|---|---|
| Persistent memory (episodic/semantic/decision/commitment) | Yes | Yes | Real (SQLite) | Yes | No | Retrieval is exact-match only (subject/goal-id/substring) — no semantic similarity, no confidence scoring, no chronological reconstruction helper, no cross-referencing decisions↔events↔entities | `IMPLEMENTABLE_NOW` (retrieval quality work) — addressed partially this pass, full synthetic-history validation (section 6) not yet done |
