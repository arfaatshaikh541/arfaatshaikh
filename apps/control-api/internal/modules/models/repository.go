...
const modelVersionColumns = `
	id, model_id, enterprise_tenant_id, version, status, provider_id, licence_id, checksum_sha256,
	...
	retention_policy, pricing_metadata, requested_by, approved_by, approved_at, rejected_reason,
...
func scanModelVersion(row pgx.Row) (ModelVersion, error) {
	...
	err := row.Scan(
		&v.ID, &v.ModelID, &v.TenantID, &v.Version, &v.Status, &v.ProviderID, &v.LicenceID, &v.ChecksumSHA256,
		...
		&retentionRaw, &pricingRaw, &v.RequestedBy, &v.ApprovedBy, &v.ApprovedAt, &v.RejectedReason,
...
func listProviders(ctx context.Context, c conn) ([]Provider, error) {
	rows, err := c.Query(ctx, `SELECT id, key, name, website, created_at FROM model_providers ORDER BY name`)
	if err != nil {
	...
	for rows.Next() {
		var p Provider
		if err := rows.Scan(&p.ID, &p.Key, &p.Name, &p.Website, &p.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
