...
func (h *Handlers) CreateOperator(w http.ResponseWriter, r *http.Request) {
	...
	op, err := h.svc.CreateOperator(r.Context(), authUser.UserID, req.LegalName, displayName, req.Country)
	if err != nil {
		apierror.WriteJSON(w, r, h.logger, apierror.Wrap(500, apierror.CodeInternal, "failed to create operator", err))
		return
	}
...
func (h *Handlers) GetOperator(w http.ResponseWriter, r *http.Request) {
	op, err := h.svc.GetOperator(r.Context())
	if err != nil {
		apierror.WriteJSON(w, r, h.logger, apierror.Wrap(500, apierror.CodeInternal, "failed to load operator", err))
		return
	}
...
func (h *Handlers) UpdateOperator(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.UpdateOperatorProfile(r.Context(), req.DisplayName); err != nil {
		apierror.WriteJSON(w, r, h.logger, apierror.Wrap(500, apierror.CodeInternal, "failed to update operator", err))
		return
	}
...
func (h *Handlers) AcceptInvitation(w http.ResponseWriter, r *http.Request) {
	...
	operatorID, err := h.svc.AcceptInvitation(r.Context(), authUser.UserID, req.Token)
	if err != nil {
	...
	writeJSON(w, http.StatusOK, map[string]any{"operator_id": operatorID})
}
