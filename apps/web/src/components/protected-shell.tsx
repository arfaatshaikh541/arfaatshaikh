...
export function ProtectedShell({ locale, children }: { locale: Locale; children: React.ReactNode }) {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Skip to content</a><header className="app-header"><Brand locale={locale}/><nav aria-label="Primary"><Link href={`/${locale}/dashboard`}>{t.dashboard}</Link><Link href={`/${locale}/organisations`}>{t.organisations}</Link><Link href={`/${locale}/source-registry`}>Source registry</Link></nav><div className="header-actions"><LocaleSwitcher locale={locale} label={t.language}/><Button className="button-quiet" onClick={async()=>{await auth.logout();router.replace(`/${locale}/login`);}}>{t.signOut}</Button></div></header><main id="main-content" className="app-main">{children}</main></div>;
}
