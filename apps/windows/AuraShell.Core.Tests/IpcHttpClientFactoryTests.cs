...
public class IpcHttpClientFactoryTests
{
    ...
    {
        var socketPath = Path.Combine(Path.GetTempPath(), $"aura-shell-test-{Guid.NewGuid():N}.sock");
        ...
        try
        {
            ...
            Assert.Contains("GET /health HTTP/1.1", requestReceived);
        }
        finally
        {
            listenSocket.Close();
    ...
    {
        var socketPath = Path.Combine(Path.GetTempPath(), $"aura-shell-test-nolisten-{Guid.NewGuid():N}.sock");
        using var client = IpcHttpClientFactory.CreateForSocket(socketPath);
