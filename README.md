...
- **`Blades.tsx`** — four procedural dark-metal claw blades (`THREE.ExtrudeGeometry` from a hand-authored `Shape`, not an external model — see "No GLB model" note below) orbiting the core at varied, asymmetric angles. Each blade has a metallic body plus a backside-additive "glow edge" duplicate for the red rim light. Scroll drives `sceneState.bladeOpen` (radius + curl), pointer/gyro parallax adds a per-blade wobble, and a click pulse gives them a brief outward kick.
- **`ElectricArcs.tsx`** — 6 procedural lightning bolts from the core toward the blades, generated with classic recursive midpoint displacement and re-rolled every ~90ms for a flicker (not a smooth glide). Intensity scales with scroll turbulence, hover, and click pulses.
- **`HoverPulseController.tsx`** — an invisible (opacity-0 but still raycastable) hit-test sphere using r3f's built-in pointer events for hover (ramps `sceneState.hoverIntensity`) and click (a GSAP "energy pulse": a quick spike in `sceneState.pulseStrength` plus a small camera punch-in via `pulseZOffset`), both consumed by the core, shell, blades, arcs, lights, and bloom.
...
**No GLB model for the blades:** the blades are fully procedural Three.js geometry, not an imported `.glb`/`.gltf`. This environment has no 3D authoring tool available to produce a real model file, and procedural geometry avoids an external binary asset, an async load/`Suspense` boundary, and a licensing question, while still being easy to reshape (the blade outline is one `THREE.Shape` bezier path in `Blades.tsx`). If you later author a real model (e.g. in Blender) and want to swap it in, drop it in `public/models/` per that folder's README, replace the `createBladeGeometry()` call with `useGLTF()` from `@react-three/drei`, and keep the rest of `Blades.tsx` (positioning, `bladeOpen`, hover/pulse response) as-is — none of that logic depends on where the geometry came from.
