...
- **`HeroEnvironment`** (in `SphereScene.tsx`) — a fully procedural reflection environment via `@react-three/drei`'s `<Environment>` + `<Lightformer>`: a neutral white panel as the dominant key light, plus a small warm rect and a cool thin strip as accents, baked into a small PMREM cubemap entirely at runtime (`resolution={128}`, `background={false}`), no HDRI file and no network fetch. This is what gives `CoreModel`'s metal blades real specular reflections instead of only flat direct-light highlights — `material.envMapIntensity` on the model is dialed down from the default 1.0 so it reads as reflective without washing out the emissive glow. The panel is neutral rather than colored on purpose: an earlier all-warm-red rig (plus red ambient/point lights and a high emissive ceiling on the heartbeat pulse in `CoreModel.tsx`) tinted the *entire* model red at rest, drowning out its actual dark-gunmetal bake — `Lighting.tsx`'s ambient and fill lights are neutral gray for the same reason, and the point lights standing in for light escaping the cracks now have a much shorter `distance` so that color stays a localized accent instead of flooding the whole silhouette.
